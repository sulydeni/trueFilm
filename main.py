...
spark = SparkSession.builder.config(
    "spark.jars", "spark-xml_2.11-0.5.0.jar"
).getOrCreate()


# load the movies csv
movies_metadata = spark.read.csv(
    "movies_metadata.csv", header=True, quote='"', escape='"'
)


# clean non digit values in revenue and budget columns
movies_metadata = (
    movies_metadata.withColumn(
        "revenue",
...
movies_metadata = movies_metadata.withColumn(
    "string_ratio", calculate_string_ratio(col("budget"), col("revenue"))
).withColumn("ratio", (col("budget") / col("revenue")))


# read wikipedia xml as spark dataframe using the spark-xml plugin
wikipedia_abstract_df = (
    spark.read.format("xml").options(rowTag="doc").load("enwiki-latest-abstract.xml")
)
...
clean_movie_abs = wikipedia_abstract_df.withColumn(
    "clean_title", regexp_extract(col("title"), "([^:]+):(.*)", 2)
).withColumn(
...
clean_movie_abs = clean_movie_abs.withColumn(
    ...
    when(
        ...
        trim(regexp_extract(col("clean_title"), r".*\(([\d]{4})[^\)]*\)$", 1)),
    ).otherwise(None),
).withColumn(
    "estimated_type",
    when(
        col("clean_title").rlike(r".*\(([\d]{4})?[^\)]*\)$"),
        trim(regexp_extract(col("clean_title"), r".*\(([\d]{4}[\s]+|)([^\)]*)\)$", 2)),
    ).otherwise(None),
)
...
clean_movie_abs = clean_movie_abs.where(
    col("clean_title_no_brackets").isin(
...
movies_metadata_with_wiki_links_test = (
    movies_metadata_with_wiki_links.withColumn(
        "match_priority",
        when(
    ...
    .orderBy(["clean_title_no_brackets", "match_priority"], ascending=False)
...
movies_metadata_with_wiki_links_test = (
    movies_metadata_with_wiki_links_test.orderBy("ratio", ascending=False)
    .limit(1000)
    .select(
        "title",
        "budget",
...
movies_metadata_with_wiki_links_final.to_sql(
    "highest_budget_revenues_ratio_movies", engine, index=False, if_exists="replace"
)
